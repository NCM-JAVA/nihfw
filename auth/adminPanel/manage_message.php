...
if(($_SESSION['logtoken']!=$random) or (!is_numeric(trim($deleteid))))
 ...
 $check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
			 ...
			 $sqd = "select image_file FROM message WHERE m_id=$cid";
					$sql="Delete From message where m_id='$deleteid'";
					$res=mysqli_query($conn,$sql);
// 					$page_id=mysqli_insert_id($conn);
					
//     $rs = mysqli_query($conn,$sqd);
...
if(($inactiveid !=''))
{
   ...
   $sql="Update message set approve_status='1' where m_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
  $sql="Update message_publish set approve_status='1' where m_publish_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
...
<?php 
	if($btnsubmit=="Search")
	{
		...
		$btneng= content_desc(check_input($btneng));
  ...
  $querywhere .=" and approve_status=$txtstatus"; 
	...
	if($btneng!='')
	  ...
	  if($errmsg=='')	
	  {
  $query ="select * from message where 1 $querywhere"; 
	  }
	 else
	{
...
// $query ="select * from message $wherecluse ";
...
<p><img alt="error" src="images/error.png"> <span>Attention! <br /></span><?php echo $_SESSION['errors']; $_SESSION['errors']="";?></p>
 ...
 <h3 class="manageuser">Manage Message</h3>
 <div class="right-section">

			 <ul>
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="message.php" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
                            ...
                            <a href="#" class="cat_link" title="View" onclick="MM_openBrWindow('messageview.php?page_id=<?php echo base64_encode($m_id);?>','window','width=1000,height=600,scrollbars=yes')">View </a> 
							</span>
							<div class="clear"></div>
							</li><?php }?>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="message.php?editid='+btoa(dataid)+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_message.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
