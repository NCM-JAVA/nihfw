...
$result = mysqli_query($conn,"select * from project where m_flag_id='".$res."' and approve_status='3'"); 
...
$check_status=check_delete($user_id,$role_id,$model_id);
			if($check_status >0)
			{
					$sql="Delete From student where m_id='$deleteid'";
					...
					if($res)
					{	
					header("location:delete.php?status=student_deleteid");
					}
			}
			else
			{
...
if(($inactiveid !=''))
{
   ...
   $sql="Update student set approve_status='1' where m_id='$inactiveid'";
 $res=mysqli_query($conn,$sql);
  $sql="Update student_publish set approve_status='1' where m_publish_id ='$inactiveid'";
 $res=mysqli_query($conn,$sql);
	if($res)
	{	
	header("location:delete_student.php?status=student_inactiveid");
	}
}
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<title>Manage Student Corner :<?php echo $sitename;?></title>
...
<div class="breadcrum">
	...
	<span class="submenuclass"> >> </span>
			 <span class="submenuclass"><a href="#">Student Corner</a></span>
			
			 <span class="submenuclass"> >> </span> 
			<span class="submenuclass"><a href="manage_student.php?sub=<?php echo $sub; ?>&txtstatus=<?php echo $txtstatus ?>"><?php echo "Manage Student Corner";?></a></span>
...
$language='2';
	  ...
	  if($errmsg=='')	
	  {
		 $query1 ="select m_id, m_name,m_title,language_id,approve_status from student  where 1 $querywhere  ORDER BY page_postion ASC";
	  }
	 else
	{
...
$wherecluse="where language_id=$language and approve_status='$txtstatus' and m_flag_id='".$sub."'";
  ...
  $query1 ="select m_id,m_name,m_title,language_id,approve_status from student $wherecluse  ORDER BY page_postion ASC";
}
function mnunam($s)
{
	require('includes/connection.php');
	$query=mysqli_query($conn,"Select m_name,m_id,m_flag_id,module_id from student where m_id='$s'");
	//echo "Select m_name,m_id,m_flag_id,module_id from student where m_id='$s'";
	 $row=mysqli_fetch_array($query);
	 if($row['m_flag_id'] >0)
	 {
		$query=mysqli_query($conn,"Select m_name,m_flag_id, module_id from student where m_id=".$row['m_flag_id']);
		//echo "Select m_name,m_flag_id, module_id from student where m_id=".$row['m_flag_id'];
		 $rows=mysqli_fetch_array($query); 
		 $model_id=$rows['module_id'];
		 

		 $val .="<a href='manage_student.php?txtstatus=3&module_id=".$model_id."'>".$rows['m_name']."</a> >> ";
	 }
	 $val .= $row['m_name'];
	echo $val;
}
...
<div class="clear"></div>
 ...
 <h3 class="manageuser">Manage Student Corner</h3>
<div class="right-section">

			 <ul>
			
<?php if($role_map['draft']=='DR' || $user_id=='101'){?><li class="new-icon">
<a href="add_student.php?module_id=<?php echo $model_id;?>" title="New"><span class="icon-28-new"></span>New</a></li>
              <?php }?>
<?php if($role_map['medit']=='ED' || $user_id=='101'){?><li id="editer" class="edit-icon"><a href="#" onclick="alert('Please select atleast one radio button!')" title="Edit">Edit</a><?php }?>
            ...
            <form id="manage_student" name="manage_student" method="post" action="">
      <div class="filter-select fltrt">          
<label class="filter-search-lbl" for="filter_search">Filter:</label>
<input id="filter_search" type="text" title="Search title or student Name." value="" name="filter_search">
<select name="txtstatus" id="txtstatus"  class="inputbox" autocomplete="off">
			...
			<?php if($role_map['pending']=='PND'){?>
					...
					<!--<li><a href="#student-1-c" onClick="document.location.href='#student-1-c';document.location.reload(false);return false">Publish</a></li>-->
					<?php }
...
<select class="inputbox" name="btneng">
 ...
 <input type="submit" name="btnsubmit" value="Search" class="button_m"/>			
    ...
    <div id="response"> </div>
							...
							<?php	
							$pager = new PS_Pagination($link, $query1,"txtstatus=".$_GET['txtstatus']."&sub=$sub");
							$rs = $pager->paginate();
							if($rs > 0){
							?>
							<ul class="">
							...
							$project_name = stripslashes($data['m_name']);
							?>
						...
						<span class="space-pagestatus"><?php if(showroot($m_id) >0) {?>
							<a href="manage_substudent.php?sub=<?php echo $m_id; ?>&module_id=<?php echo $model_id;?>&txtstatus=<?php echo $_GET['txtstatus'];?>" title="Sub student View"><?php status($approve_status);?></a><?php } else { status($approve_status); }?></span>
                          ...
                          <?php   //if($role_map['publish']=='PB' || $txtstatus=='3'){?>
							<a href="manage_student.php?inactiveid=<?php echo $m_id; ?>&module_id=<?php echo $model_id;?>&&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm('Are you sure you want to Inactive this page?')">Inactive</a>&nbsp;/&nbsp;<?php //}?>
                            <a href="#project-1-c" class="cat_link" title="View" onclick="MM_openBrWindow('studentview.php?page_id=<?php echo $m_id;?>','window','width=600,height=500,scrollbars=yes')">View </a>
							</span>
							<div class="clear"></div>
							</li><?php }?>
							</ul>
...
function editlist(id) {
    ...
    jQuery.ajax({
		...
		{
			var e='<a href="edit_student.php?editid='+dataid+'" title="Edit"><span class="icon-28-edit"></span>Edit</a>';
			var d='<a href="manage_student.php?deleteid='+dataid+'&random=<?php echo $_SESSION['logtoken'];?>" onclick="return confirm(\'Are you sure you want to delete this record permanently?\');" title="Delete"><span class="icon-28-delete"></span>Delete</a>';
