...
require_once "../includes/functions-cms.inc.php";
include('../../design.php');
...
foreach($contentrows as $key=>$value){  
		...
		if($m_url=='notifications.php')
        {
        
        header("Location:".$HomeURL."/notifications.php");
        exit();
...
if($page_id=='47' || $page_id=='336')
    ...
    $query = mysqli_query($conn,"select page_action_date as lastupdate from audit_trail where page_id=$page_id ORDER BY audit_id DESC LIMIT 0, 1");
...
?>
 ...
 <li class="ook">
      ...
      <ul  class="ddsubmenustyle blackwhite" style="display: none;">
        ...
        $sql2 = mysqli_query($conn,"select * from menu_publish where m_flag_id='".$value1['m_publish_id']."' and menu_positions='3' and language_id='1' and approve_status='3' ORDER BY page_postion ASC");
...
?>
  <?php foreach($leftrows as $key=>$value){
      ...
      $sql1 = mysqli_query($conn,"select * from menu_publish where m_flag_id='".$value['m_publish_id']."' and menu_positions='2' and language_id='1' and approve_status='3' ORDER BY page_postion ASC");
      //echo $value['m_name']."".
      $row2 = mysqli_num_rows($sql1);
         if($row2 > 0){ ?>
      ...
      <ul  class="ddsubmenustyle blackwhite">
        ...
        $sql2 = mysqli_query($conn,"select * from menu_publish where m_flag_id='".$value1['m_publish_id']."' and menu_positions='2' and language_id='1' and approve_status='3' ORDER BY page_postion ASC");
        $row3 = mysqli_num_rows($sql2);
       if($row3 > 0){
        ?>
...
?>
     ...
     <?php if($value1['doc_uplode']!='') {?>
			...
			<?php } else { ?>
		<li><a href="<?php echo $HomeURL;?><?php echo $value1['m_url']; ?>" title="<?php echo $value1['m_name'];?>"><?php echo $value1['m_name'];?></a></li>
