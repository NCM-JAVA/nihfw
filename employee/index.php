...
if($_SERVER['REQUEST_URI'])
		...
		$url1=$val['3'];
		$open=$val['2'];
		 if($mydb->checkTableRow("emp_category")>0){
					$i=1;
					$date=date('Y-m-d');
			 		$whereClause="status='1' && language_id='1'";
					$whereClause .=  '&& page_url="'.$url1.'"';
					$newsrows=$mydb->gettable_Rows_whereCluse("emp_category",$whereClause);
				}
...
<div class="col-md-9 content-area">
                <h2 class="heading"><?php echo $page_name;?></h2>
                	 ...
                	 <a href="http://117.239.177.100/nihfw/archive.php?cat=7" style="float: right;"  title="&nbsp;<strong>Archive</strong>">&nbsp;<strong>Archive</strong>&nbsp;<img src="http://117.239.177.100/nihfw/images/extlink.png" alt="external link"></a>
                	 <br>
			   ...
			   <th>Last Date</th>
			    ...
			    <?php if($mydb->checkTableRow("employee_corner_publish")>0){
					$i=1;
					$date=date('Y-m-d');
			/*$whereClause="approve_status='3' && language_id='1' && category='".$cat."' && date(end_date ) >= '$date'  order by start_date desc" ;*/

			$whereClause="approve_status='3' && language_id='1' && category='".$cat."'    order by start_date desc" ;
					
					$newsrows=$mydb->gettable_Rows_whereCluse("employee_corner_publish",$whereClause);

					//echo $mm = 'select * from employee_corner_publish where '.$whereClause;
				...
				?>
				 <?php
					 if($no_of_rows > 0){
					foreach($newsrows as $key=>$value){ 
					   $docspath = '../upload/employee/'.$value['docs_file'];
			   ...
			   <a href="<?php echo $docspath; ?>" title="<?php echo $value['m_name'];?>" target="_blank"><?php echo $value['m_title'];?>  &nbsp;&nbsp;<img src="<?php echo $HomeURL; ?>/images/pdf_icon.png" height="16" alt="PDF file" />   <?php echo ' size:( '.formatFilebytes($docspath ,'MB').')'; ?></a>
			   ...
			   <a href="<?php echo $value['ext_url']; ?>" title="<?php echo $value['m_name'];?>" target="_blank"><?php echo $value['m_title'];?>  &nbsp;&nbsp;<img src="<?php echo $HomeURL; ?>/images/extlink.png" height="16" alt="PDF file1" /> </a>
			   <?php } else { ?>
