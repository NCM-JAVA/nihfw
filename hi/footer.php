<div class="row">
		...
		if($mydb->checkTable_threeRow("menu_publish","m_flag_id",0,"menu_positions",3,"approve_status",3)>0){
			...
			$whereClause="m_flag_id='0' && menu_positions='3' && approve_status='3' && language_id='2' and m_publish_id!='290'order by menu_positions asc limit 0,4" ;
...
<?php foreach($leftrows as $key=>$value){
			...
			$whereClause="m_flag_id='".$value['m_publish_id']."' && menu_positions='3' && approve_status='3' && language_id='2' order by menu_positions asc limit 0,6" ;
		...
		<li><a href="<?php echo $HomeURL;?>/hi/cms/<?php echo $value1['m_url']; ?>" title="<?php echo $value1['m_name'];?>"><?php echo $value1['m_name'];?></a></li>
              ...
              <div class="v-all"><a href="<?php echo $HomeURL;?>/view-all.php?menu=<?php echo $value['m_name'];?>" title="सभी देखें">सभी देखें</a></div>
...
<?php foreach($leftrows as $key=>$value){
			...
			//$leftrows1=$mydb->gettable_RowsfourColumn_where("menu_publish","m_flag_id",$value['m_publish_id'],"menu_positions",3,"approve_status",3,"language_id",1);
			$whereClause="m_flag_id='".$value['m_publish_id']."' && menu_positions='1' && approve_status='3' && language_id='2' order by page_postion asc limit 0,6" ;
			$leftrows1=$mydb->gettable_Rows_whereCluse("menu_publish",$whereClause); 
