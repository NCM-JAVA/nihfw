...
<?php $sqlquery=mysqli_query($conn,"select * from message_publish where approve_status=3 limit 0,1");
							$i=1;
							...
							?>
                <h2 class="heading-whats-new">From the Director's Desk</h2>
            		<!--<img src="<?php echo $image_path;?>" alt="NIHFW Director" title="EPI" class="NIHFW Director">-->
                    <p><?php echo limit_words($m_description,30);?>
					<span style="font-size:13px;padding-left:9px;">(<?php echo ($designation);?>)</span><span><?php echo $m_name;?> </span>
					</p>
                     
    <div class="director-v-all" style="float:right !important"><a href="<?php echo $HomeURL;?>/director_message.php" title="View All">Read more...</a></div>
              	</div>
                <?php } ?>
            	
            	<div class="director-desk">
                <h2 class="heading-metting">Meetings &amp; Events <!-- / Workshop &amp; Training --></h2>
				<?php
  $date=date('Y-m-d');
...
// $whereClause="approve_status='3' && language_id='1' && date(end_date ) >= '$date'  order by start_date desc" ;
 ...
 ?>
    ...
    <div class="director-v-all"><a href="<?php echo $HomeURL;?>/meeting.php" title="View All">Read more...</a></div>
              	</div>
