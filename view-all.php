...
include('../counter.php');
	...
	if($mainmenu != 'ContactUs'){
		...
		$mp = 3;
		//$whereClause = "m_flag_id=0 && menu_positions=3 && approve_status=3 && language_id=1 and m_publish_id!=19 and m_name = '$mainmenu' " ;
		$whereClause = "m_flag_id='0' && menu_positions='3' && approve_status='3' && language_id='1' and m_publish_id!='19' and m_name = '$mainmenu' " ;
	}else{
	
		$mp = 1;
		// $whereClause = "m_flag_id=0 && menu_positions=1 && approve_status=3 && language_id=1 and m_publish_id=7 order by page_postion asc limit 0,1" ;
	$whereClause = "m_flag_id='0' && menu_positions='1' && approve_status='3' && language_id='1' and m_publish_id='7' order by page_postion asc limit 0,1" ;
	}
	
	$supermenu 	 = $mydb->gettable_Rows_whereCluse("menu_publish",$whereClause); 
	
	if(strcmp($mainmenu,'Contact Us') === -53){
		
		$mflagid = 19;
	}else{
		//echo 'here';
		$mflagid = $supermenu[0]['m_publish_id'];
	}
	//echo 'mflag'.$mflagid;
	$whereClauseSub 	= "m_flag_id=".$mflagid." && menu_positions=$mp && approve_status=3 && language_id=1 order by page_postion asc " ;
	//$whereClauseSub 	= "menu_positions=$mp && approve_status=3 && language_id=1 order by page_postion asc " ;
